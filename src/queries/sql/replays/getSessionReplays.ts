...
async function clickhouseQuery(websiteId: string, filters: QueryFilters, sessionId?: string) {
  ...
  const havingQuery = minDurationMs
    ? `having toUnixTimestamp64Milli(max(session_replay.ended_at)) - toUnixTimestamp64Milli(min(session_replay.started_at)) >= {minDurationMs:Int64}`
    : '';
  ...
  return pagedRawQuery(
    ...
    select
      ...
      toUnixTimestamp64Milli(max(session_replay.ended_at)) - toUnixTimestamp64Milli(min(session_replay.started_at)) as duration,
